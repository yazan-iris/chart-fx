...
public class Clipboard implements EventSource, EventListener {
    ...
    @OpenApi(
            description = "clipboard root returns either a list (HTML: overview) or content of specific data element",
            operationId = "exportHandler",
            summary = "My Summary",
            deprecated = false,
            tags = { "Clipboard" },
    ...
    private final Handler exportHandler
            = new CombinedHandler(ctx -> {
                  System.err.println("called exportHandler with path = " + ctx.path());
    ...
    //            operationId = "createExportHandler",
    //            summary = "My Summary",
    //            deprecated = false,
    //            tags = { "Clipboard" },
    //            path = ENDPOINT_CLIPBOARD,
    //            //            method = HttpMethod.GET,
    //            //requestBody = @OpenApiRequestBody(content = @OpenApiContent(from = Clipboard.class)),
    //            //requestBody = @OpenApiRequestBody(content = {@OpenApiContent(type = "text/html") , @OpenApiContent(type = "image/png")}),
    //            headers = @OpenApiParam(name = "my-custom-header"),
    //            responses = {
    ...
    //            })
    //    private Handler
    //    createExportHandler(Context ctx) {
    ...
    @OpenApi(
            description = "endpoint to receive clipboard data",
            operationId = "uploadHandlerPost",
            summary = "POST",
            deprecated = false,
            tags = { "Clipboard" },
    ...
    public Clipboard(final String exportRoot, final String exportName, final Region regionToCapture, final long maxUpdatePeriod, final TimeUnit maxUpdatePeriodTimeUnit, final boolean allowUploads) {
        ...
        RestServer.getInstance().get(exportRoot, new CombinedHandler(ctx -> serveCategoryOverview(ctx, fixPreAndPost(CLIPBOARD_ROOT))), accessRoles);
        RestServer.getInstance().get(prefixPath(ENDPOINT_CLIPBOARD), exportHandler, accessRoles);
        //        RestServer.getInstance().get(prefixPath(ENDPOINT_CLIPBOARD), this::createExportHandler, accessRoles);
